...
export function createContractsQueries<
  ...
  TRelease extends { version: string; deployments: Array<{ chainId: number; contracts: TContract[] }> },
  TCatalog,
>(config: {
  catalog: TCatalog;
  releasesQueries: {
  ...
  return {
    ...
    get: (opts: {
      ...
      if (contractName) {
        ...
        return dep ? (_.find(dep.contracts, { name: contractName }) as TContract | undefined) : undefined;
      }
      ...
      if (contractAddress) {
        ...
        if (release) {
          const dep = _.find(release.deployments, { chainId });
          return dep
            ? (_.find(dep.contracts, (c) => normalizeAddress(c.address) === address) as TContract | undefined)
            : undefined;
        }
        ...
        if (protocol) {
          ...
          const matches = releases.filter((rel) => {
            ...
            return dep && _.some(dep.contracts, (c) => normalizeAddress(c.address) === address);
          });
