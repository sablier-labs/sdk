...
export const releasesQueries = {
   ...
   */
  getAll: (opts?: { protocol?: Sablier.Solana.Protocol }): Sablier.Solana.Release[] => {
    const { protocol } = opts || {};
    if (protocol) {
   ...
   */
  getFirst: (opts: { protocol: Sablier.Solana.Protocol; chainId?: number }): Sablier.Solana.Release | undefined => {
   ...
   * - {protocol}
   */
  getLatest: (opts: { protocol: Sablier.Solana.Protocol }): Sablier.Solana.Release => {
