...
describe("Indexed contracts have a deployment block number", () => {
  for (const release of sablier.solanaReleases.getAll()) {
    describe(`${release.protocol} ${release.version}`, () => {
      const contracts = sablier.solanaContracts.getAll({ release })!;
      ...
      for (const contract of contracts) {
        ...
        const chain = sablier.solanaChains.getOrThrow(contract.chainId);
